...
import { LogLevel, middleware } from "./esm/middleware.js";
...
const test = async () => {
	...
	await fs.writeFile(path.join(dir, "page.css"), "h1{color:#F00;}");
	await fs.writeFile(
	...
	await Promise.all([
		fs.writeFile(path.join(dir, "page.css"), "h1{color:#00F;}"),
		fileEvents.waitForEvent("change", /page\.css$/, 10000),
	]);
	const startedAt = Date.now();
	const timeoutMs = 3000;
	while (timeoutMs < Date.now() - startedAt) {
